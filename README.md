...
- Для числа переменных от 2 до 100 генерируются серии формул.
- Для каждой пары измеряется время работы брутфорса, DPLL, CDCL и Z3, затем для каждого значения $n$ усредняется время и результаты сохраняются в файл `benchmark_results.csv`.
...
- Оси: по X — число входных переменных, по Y — среднее время в миллисекундах (логарифмическая шкала); график также сохраняется в файл `benchmark_plot.png`.
...
- `main.py` — ядро проекта:
    ...
    - реализация методов проверки эквивалентности: `check_equiv_bruteforce` (полный перебор), `check_equiv_dpll` (учебный DPLL), `check_equiv_cdcl` (учебный CDCL) и `check_equiv_z3` (митор + Z3);
    - генерация случайных формул и функция `demo()` с набором тестов.
- `manual_test.py` — интерактивный режим: чтение формул с консоли, проверка синтаксиса через парсер из `main.py`, запуск методов проверки и вывод результатов пользователю.
- `benchmark.py` — автоматический бенчмарк: генерация серий случайных формул для разных чисел переменных, замер и усреднение времени работы методов, сохранение результатов в CSV.
- `plot_benchmark.py` — построение графика по файлу `benchmark_results.csv` с логарифмической шкалой по оси времени и сохранением изображения в `benchmark_plot.png`.

## Дополнительные файлы

- `benchmark_plot.png` — график зависимости среднего времени выполнения от количества переменных.
- `benchmark_results.csv` — полная таблица с результатами бенчмаркинга.
- `manual_test_results.txt` — логи ручного тестирования.